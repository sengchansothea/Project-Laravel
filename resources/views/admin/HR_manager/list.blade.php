    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>​បញ្ជីឈ្មោះ HR Manager <small style="color: rgb(0, 60, 255)">({{ $getRecord->count() }} នាក់)</small></h1>
                    </div>
                    <div class="col-sm-6 text-right">
                        <a href="{{ url('admin/HR_manager/add') }}" class="btn btn-primary">
                            <i class="fas fa-plus"></i> បន្ថែម HR Manager
                        </a>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-3 text-right">
                                            ...
                                            <a href="{{ url('admin/HR_manager/list') }}" class="btn btn-success" style="margin-top: 32px;">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-tools text-right mb-2">
                            <a href="{{ url('admin/HR_manager/export-csv') }}" class="btn btn-outline-primary btn-sm">
                                <i class="fas fa-file-csv"></i> CSV
                            </a>
                            <a href="{{ url('admin/HR_manager/export-excel') }}" class="btn btn-outline-success btn-sm">
                                <i class="fas fa-file-excel"></i> Excel
                            </a>
                            <a href="{{ url('admin/HR_manager/export-pdf') }}" class="btn btn-outline-danger btn-sm">
                    ...
                    <div class="card-body">
                        <div style="overflow-x: auto; width: 100%;">
                            <table class="table table-bordered" style="white-space: nowrap;">
                                ...
                                <tbody>
                                    @foreach ($getRecord as $HR_manager)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td class="center-image">
                                                @if (!empty($HR_manager->getProfile()))
                                                    <a href="{{ url('admin/profile', ['id' => $HR_manager->id]) }}">
                                                        <img src="{{ $HR_manager->getProfile() }}" class="img-circle">
                                                    </a>
                                                @endif
                                            </td>
                                            <td>{{ $HR_manager->name }} {{ $HR_manager->last_name }}</td>
                                            <td>{{ $HR_manager->gender == 'Male' ? 'ប្រុស' : ($HR_manager->gender == 'Female' ? 'ស្រី' : 'មិនបានបញ្ជាក់') }}</td>
                                            <td>{{ $HR_manager->email }}</td>
                                            <td>{{ $HR_manager->phone_number }}</td>                                   
                                            <td class="wrap-text">
                                                @if (!empty($HR_manager->date_of_birth))
                                                    {{ date('d-m-Y', strtotime($HR_manager->date_of_birth)) }}
                                                @endif
                                            </td>
                                            <td class="wrap-text">
                                                @if (!empty($HR_manager->date_of_joining))
                                                    {{ date('d-m-Y', strtotime($HR_manager->date_of_joining)) }}
                                                @endif
                                            </td>
                                            <td>
                                                @if($HR_manager->marital_status == 'Single')
                                                    អនីតិជន
                                                @elseif($HR_manager->marital_status == 'Married')
                                                    បានរៀបការ
                                                @elseif($HR_manager->marital_status == 'Other')
                                                    មិនបានបញ្ជាក់
                                                @endif
                                            </td>
                                            <td>{{ $HR_manager->province_kh }}</td>
                                            <td>{{ $HR_manager->address}}</td>
                                            <td>{{ $HR_manager->qualification}}</td>
                                            <td>{{ $HR_manager->work_experience}}</td>
                                            <td>
                                                <small class="{{ $HR_manager->status == 0 ? 'status-active' : 'status-inactive' }}">
                                                    {{ $HR_manager->status == 0 ? 'បានអនុម័ត' : 'កំពុងរង់ចាំ' }}
                                                </small>
                                            </td>
                                            <td>{{ $HR_manager->created_at->format('d-m-Y H:i A') }}</td>
                                            <td>
                                                <a href="{{ url('admin/HR_manager/edit/' . $HR_manager->id) }}" class="btn btn-primary btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="{{ url('admin/HR_manager/delete/' . $HR_manager->id) }}" class="btn btn-danger btn-sm">
                                                    <i class="fas fa-trash"></i> Delete
                                                </a>
                                                {{-- <a href="{{ url('admin/HR_manager/approve/' . $HR_manager->id) }}" class="btn btn-success btn-sm">
