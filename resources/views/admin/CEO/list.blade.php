    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>​បញ្ជីឈ្មោះ CEO <small style="color: rgb(0, 60, 255)">({{ $getRecord->count() }} នាក់)</small></h1>
                    </div>
                    <div class="col-sm-6 text-right">
                        <a href="{{ url('admin/CEO/add') }}" class="btn btn-primary">
                            <i class="fas fa-plus"></i> បន្ថែម CEO
                        </a>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-3 text-left">
                                            ...
                                            <a href="{{ url('admin/CEO/list') }}" class="btn btn-success" style="margin-top: 32px;">
                    ...
                    <div class="card-body">
                        <div style="overflow-x: auto; width: 100%;">
                            <table class="table table-bordered" style="white-space: nowrap;">
                                ...
                                <tbody>
                                    @foreach ($getRecord as $CEO)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td class="center-image">
                                                @if (!empty($CEO->getProfile()))
                                                    <a href="{{ url('admin/profile', ['id' => $CEO->id]) }}">
                                                        <img src="{{ $CEO->getProfile() }}" class="img-circle">
                                                    </a>
                                                @endif
                                            </td>
                                            <td>{{ $CEO->name }} {{ $CEO->last_name }}</td>
                                            <td>{{ $CEO->gender == 'Male' ? 'ប្រុស' : ($CEO->gender == 'Female' ? 'ស្រី' : 'មិនបានបញ្ជាក់') }}</td>
                                            <td>{{ $CEO->email }}</td>
                                            <td>{{ $CEO->phone_number }}</td>                                     
                                            <td class="wrap-text">
                                                @if (!empty($CEO->date_of_birth))
                                                    {{ date('d-m-Y', strtotime($CEO->date_of_birth)) }}
                                                @endif
                                            </td>
                                            <td class="wrap-text">
                                                @if (!empty($CEO->date_of_joining))
                                                    {{ date('d-m-Y', strtotime($CEO->date_of_joining)) }}
                                                @endif
                                            </td>
                                            <td>
                                                @if($CEO->marital_status == 'Single')
                                                    អនីតិជន
                                                @elseif($CEO->marital_status == 'Married')
                                                    បានរៀបការ
                                                @elseif($CEO->marital_status == 'Other')
                                                    មិនបានបញ្ជាក់
                                                @endif
                                            </td>
                                            <td>{{ $CEO->province_kh }}</td>
                                            <td>{{ $CEO->address}}</td>
                                            <td>{{ $CEO->qualification}}</td>
                                            <td>{{ $CEO->work_experience}}</td>
                                            <td>
                                                <small class="{{ $CEO->status == 0 ? 'status-active' : 'status-inactive' }}">
                                                    {{ $CEO->status == 0 ? 'បានអនុម័ត' : 'កំពុងរង់ចាំ' }}
                                                </small>
                                            </td>
                                            <td>{{ $CEO->created_at->format('d-m-Y H:i A') }}</td>
                                            <td>
                                                <a href="{{ url('admin/CEO/edit/' . $CEO->id) }}" class="btn btn-primary btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="{{ url('admin/CEO/delete/' . $CEO->id) }}" class="btn btn-danger btn-sm">
                                                    <i class="fas fa-trash"></i> Delete
                                                </a>
                                                {{-- <a href="{{ url('admin/CEO/approve/' . $CEO->id) }}" class="btn btn-success btn-sm">
