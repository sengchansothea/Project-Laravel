    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>​បញ្ជីឈ្មោះ CFO <small style="color: rgb(0, 60, 255)">({{ $getRecord->count() }} នាក់)</small></h1>
                    </div>
                    <div class="col-sm-6 text-right">
                        <a href="{{ url('admin/CFO/add') }}" class="btn btn-primary">
                            <i class="fas fa-plus"></i> បន្ថែម CFO
                        </a>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-3 text-right">
                                            ...
                                            <a href="{{ url('admin/CFO/list') }}" class="btn btn-success" style="margin-top: 32px;">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-tools text-right mb-2">
                            <a href="{{ url('admin/CFO/export-csv') }}" class="btn btn-outline-primary btn-sm">
                                <i class="fas fa-file-csv"></i> CSV
                            </a>
                            <a href="{{ url('admin/CFO/export-excel') }}" class="btn btn-outline-success btn-sm">
                                <i class="fas fa-file-excel"></i> Excel
                            </a>
                            <a href="{{ url('admin/CFO/export-pdf') }}" class="btn btn-outline-danger btn-sm">
                    ...
                    <div class="card-body">
                        <div style="overflow-x: auto; width: 100%;">
                            <table class="table table-bordered" style="white-space: nowrap;">
                                ...
                                <tbody>
                                    @foreach ($getRecord as $CFO)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td class="center-image">
                                                @if (!empty($CFO->getProfile()))
                                                    <a href="{{ url('admin/profile', ['id' => $CFO->id]) }}">
                                                        <img src="{{ $CFO->getProfile() }}" class="img-circle">
                                                    </a>
                                                @endif
                                            </td>
                                            <td>{{ $CFO->name }} {{ $CFO->last_name }}</td>
                                            <td>{{ $CFO->gender == 'Male' ? 'ប្រុស' : ($CFO->gender == 'Female' ? 'ស្រី' : 'មិនបានបញ្ជាក់') }}</td>
                                            <td>{{ $CFO->email }}</td>
                                            <td>{{ $CFO->phone_number }}</td>                                     
                                            <td class="wrap-text">
                                                @if (!empty($CFO->date_of_birth))
                                                    {{ date('d-m-Y', strtotime($CFO->date_of_birth)) }}
                                                @endif
                                            </td>
                                            <td class="wrap-text">
                                                @if (!empty($CFO->date_of_joining))
                                                    {{ date('d-m-Y', strtotime($CFO->date_of_joining)) }}
                                                @endif
                                            </td>
                                            <td>
                                                @if($CFO->marital_status == 'Single')
                                                    អនីតិជន
                                                @elseif($CFO->marital_status == 'Married')
                                                    បានរៀបការ
                                                @elseif($CFO->marital_status == 'Other')
                                                    មិនបានបញ្ជាក់
                                                @endif
                                            </td>
                                            <td>{{ $CFO->province_kh }}</td>
                                            <td>{{ $CFO->address}}</td>
                                            <td>{{ $CFO->qualification}}</td>
                                            <td>{{ $CFO->work_experience}}</td>
                                            <td>
                                                <small class="{{ $CFO->status == 0 ? 'status-active' : 'status-inactive' }}">
                                                    {{ $CFO->status == 0 ? 'បានអនុម័ត' : 'កំពុងរង់ចាំ' }}
                                                </small>
                                            </td>
                                            <td>{{ $CFO->created_at->format('d-m-Y H:i A') }}</td>
                                            <td>
                                                <a href="{{ url('admin/CFO/edit/' . $CFO->id) }}" class="btn btn-primary btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="{{ url('admin/CFO/delete/' . $CFO->id) }}" class="btn btn-danger btn-sm">
                                                    <i class="fas fa-trash"></i> Delete
                                                </a> 
                                                {{-- <a href="{{ url('admin/CFO/approve/' . $CFO->id) }}" class="btn btn-success btn-sm">
