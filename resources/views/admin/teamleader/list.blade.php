    ...
    <div class="content-wrapper">
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>​បញ្ជីឈ្មោះ Team Leader <small style="color: rgb(0, 60, 255)">({{ $getRecord->count() }} នាក់)</small></h1>
                    </div>
                    <div class="col-sm-6 text-right">
                        <a href="{{ url('admin/teamLeader/add') }}" class="btn btn-primary">
                            <i class="fas fa-plus"></i> បន្ថែម Team Leader
                        </a>
                    </div>
                </div>
            </div>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-3 text-right">
                                            ...
                                            <a href="{{ url('admin/teamLeader/list') }}" class="btn btn-success" style="margin-top: 32px;">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="card-tools text-right mb-2">
                            <a href="{{ url('admin/teamLeader/export-csv') }}" class="btn btn-outline-primary btn-sm">
                                <i class="fas fa-file-csv"></i> CSV
                            </a>
                            <a href="{{ url('admin/teamLeader/export-excel') }}" class="btn btn-outline-success btn-sm">
                                <i class="fas fa-file-excel"></i> Excel
                            </a>
                            <a href="{{ url('admin/teamLeader/export-pdf') }}" class="btn btn-outline-danger btn-sm">
                    ...
                    <div class="card-body">
                        <div style="overflow-x: auto; width: 100%;">
                            <table class="table table-bordered" style="white-space: nowrap;">
                                ...
                                <tbody>
                                    @foreach ($getRecord as $teamLeader)
                                        <tr>
                                            <td>{{ $loop->iteration }}</td>
                                            <td class="center-image">
                                                @if (!empty($teamLeader->getProfile()))
                                                    <a href="{{ url('admin/profile', ['id' => $teamLeader->id]) }}">
                                                        <img src="{{ $teamLeader->getProfile() }}" class="img-circle">
                                                    </a>
                                                @endif
                                            </td>
                                            <td>{{ $teamLeader->name }} {{ $teamLeader->last_name }}</td>
                                            <td>{{ $teamLeader->gender == 'Male' ? 'ប្រុស' : ($teamLeader->gender == 'Female' ? 'ស្រី' : 'មិនបានបញ្ជាក់') }}</td>
                                            <td>{{ $teamLeader->email }}</td>
                                            <td>{{ $teamLeader->phone_number }}</td>                                   
                                            <td class="wrap-text">
                                                @if (!empty($teamLeader->date_of_birth))
                                                    {{ date('d-m-Y', strtotime($teamLeader->date_of_birth)) }}
                                                @endif
                                            </td>
                                            <td class="wrap-text">
                                                @if (!empty($teamLeader->date_of_joining))
                                                    {{ date('d-m-Y', strtotime($teamLeader->date_of_joining)) }}
                                                @endif
                                            </td>
                                            <td>
                                                @if($teamLeader->marital_status == 'Single')
                                                    អនីតិជន
                                                @elseif($teamLeader->marital_status == 'Married')
                                                    បានរៀបការ
                                                @elseif($teamLeader->marital_status == 'Other')
                                                    មិនបានបញ្ជាក់
                                                @endif
                                            </td>
                                            <td>{{ $teamLeader->province_kh }}</td>
                                            <td>{{ $teamLeader->address}}</td>
                                            <td>{{ $teamLeader->qualification}}</td>
                                            <td>{{ $teamLeader->work_experience}}</td>
                                            <td>
                                                <small class="{{ $teamLeader->status == 0 ? 'status-active' : 'status-inactive' }}">
                                                    {{ $teamLeader->status == 0 ? 'បានអនុម័ត' : 'កំពុងរង់ចាំ' }}
                                                </small>
                                            </td>
                                            <td>{{ $teamLeader->created_at->format('d-m-Y H:i A') }}</td>
                                            <td>
                                                <a href="{{ url('admin/teamLeader/edit/' . $teamLeader->id) }}" class="btn btn-primary btn-sm">
                                                    <i class="fas fa-edit"></i> Edit
                                                </a>
                                                <a href="{{ url('admin/teamLeader/delete/' . $teamLeader->id) }}" class="btn btn-danger btn-sm">
                                                    <i class="fas fa-trash"></i> Delete
                                                </a>
                                                {{-- <a href="{{ url('admin/teamLeader/approve/' . $teamLeader->id) }}" class="btn btn-success btn-sm">
