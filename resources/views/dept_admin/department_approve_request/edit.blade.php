                                    ...
                                    <div class="form-group">
                                        ...
                                        @foreach ($getDepartment as $department)
                                            @php
                                                $checked = '';
                                            @endphp
                                            @foreach ($getAssignUserID as $Department)
                                                @if ($Department->dapartment_id == $department->id)
                                                    @php
                                                        $checked = 'checked';
                                                    @endphp
                                                @endif
                                            ...
                                            <div>
                                                <label style="font-weight: normal">
                                                    <input {{ $checked }} type="checkbox" value="{{ $department->id }}"
                                                        name="dapartment_id[]"> {{ $department->name }}
                                                </label>
                                            </div>
                                        @endforeach
                                    </div>
                                    <div class="form-group">
                                        <label>ប្រភេទនៃការសើ្នសុំ</label>
                                        @foreach ($getTypeRequest as $typerequest)
                                            @php
                                                $checked = '';
                                            @endphp
                                            @foreach ($getAssignUserID as $typeRequest)
                                                @if ($typeRequest->type_request_id == $typerequest->id)
                                                    @php
                                                        $checked = 'checked';
                                                    @endphp
                                                @endif
                                            ...
                                            <div>
                                                <label style="font-weight: normal">
                                                    <input {{ $checked }} type="checkbox"
                                                        value="{{ $typerequest->id }}" name="type_request_id[]">
                                                    {{ $typerequest->name }}
                                                </label>
                                            </div>
                                        @endforeach
